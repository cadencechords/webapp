...
const CHROMATIC = ['red', 'blue', 'green', 'yellow', 'pink', 'purple', 'indigo'];
const EVENT_COLORS = ['red', 'blue', 'yellow', 'green', 'pink', 'purple', 'indigo', 'gray', 'black']; // EventColorOptions.js
...
test('covers every color the app stores', () => {
  const stored = new Set([...BINDER_COLORS.filter(c => c !== 'none'), ...EVENT_COLORS, ...NOTE_COLORS]);
...
describe.each(USER_COLOR_NAMES)('%s', name => {
  ...
  test.each([['light', light], ['dark', dark]])('%s text meets 4.5:1', (_, c) => {
    expect(contrast(c['on-color'], c.color)).toBeGreaterThanOrEqual(4.5);
    expect(contrast(c['on-container'], c.container)).toBeGreaterThanOrEqual(4.5);
  });
});

test.each(CHROMATIC)('%s keeps its hue (within 10 degrees)', name => {
  expect(hueDistance(hue(generateUserColor(name).light.color), hue(USER_COLORS[name]))).toBeLessThanOrEqual(10);
});

test('chromatic colors stay distinguishable from each other', () => {
  for (const a of CHROMATIC) {
    for (const b of CHROMATIC) {
      if (a < b) expect(hueDistance(hue(generateUserColor(a).light.color), hue(generateUserColor(b).light.color)), `${a}/${b}`).toBeGreaterThanOrEqual(10);
    }
  }
});
...
test('gray and black are near-neutral', () => {
  for (const name of ['gray', 'black']) {
    expect(Hct.fromInt(argbFromHex(generateUserColor(name).light.container)).chroma).toBeLessThan(8);
  }
});
...
describe('userColorClasses', () => {
  test('unknown, empty and "none" fall back to neutral roles', () => {
    for (const name of ['none', '', undefined, 'teal']) {
      expect(userColorClasses(name).container).toBe('bg-surface-container-highest');
    }
  });
  ...
  test('every class compiles to its token', async () => {
    ...
    const classes = USER_COLOR_NAMES.flatMap(n => Object.values(userColorClasses(n)));
    const css = compiler.build(classes);
    for (const n of USER_COLOR_NAMES) {
      expect(css).toMatch(new RegExp(`\\.bg-user-${n}-container\\s*\\{[^}]*var\\(--md-custom-color-${n}-container\\)`));
      expect(css).toMatch(new RegExp(`\\.text-on-user-${n}\\s*\\{[^}]*var\\(--md-custom-color-on-${n}\\)`));
    }
  });
});
