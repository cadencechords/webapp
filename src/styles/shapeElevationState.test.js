...
beforeAll(async () => {
  ...
  css = compiler.build([
    'rounded-extra-small', 'rounded-large-increased', 'rounded-extra-extra-large', 'rounded-lg', 'rounded-sm',
    'elevation-0', 'elevation-3', 'elevation-5', 'state-layer', 'focus-ring',
  ]);
});
...
test('state-layer overlays currentColor at the M3 opacities', () => {
  ...
  for (const [state, token] of [['&:hover', 'hover'], ['&:focus-visible', 'focus'], ['&:active', 'pressed'], ['&[data-dragging]', 'dragged']]) {
    expect(block).toMatch(new RegExp(`${state.replace(/[[\]]/g, '\\$&')}::before\\s*\\{\\s*opacity: var\\(--md-sys-state-${token}-state-layer-opacity\\)`));
  }
...
test('focus-ring draws a secondary outline on keyboard focus', () => {
  expect(css).toMatch(/\.focus-ring:focus-visible\s*\{[^}]*outline: var\(--md-sys-state-focus-indicator-thickness\) solid var\(--md-sys-color-secondary\)/);
});
