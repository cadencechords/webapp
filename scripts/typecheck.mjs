...
let output = '';
try {
  output = execFileSync('npx', ['tsc', '-p', '.', '--pretty', 'false'], { encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
} catch (error) {
...
if (update) {
  ...
  console.log(`Baseline updated: ${errors.length} errors in ${Object.keys(sorted).length} files.`);
  process.exit(0);
}

const baseline = JSON.parse(readFileSync(BASELINE, 'utf8'));
const regressions = Object.entries(sorted).filter(([file, n]) => n > (baseline[file] ?? 0));
const improved = Object.entries(baseline).filter(([file, n]) => (sorted[file] ?? 0) < n);
...
if (regressions.length) {
  ...
  for (const [file, n] of regressions) {
    ...
    for (const line of errors.filter(l => l.startsWith(`${file}(`))) console.error(`    ${line.slice(file.length)}`);
  }
  console.error('\nFix them, or if a count went up for a good reason, run `yarn typecheck --update`.');
  process.exit(1);
}

console.log(`Typecheck OK: ${errors.length} known errors in ${Object.keys(sorted).length} files, none new.`);
if (improved.length) {
  console.log(`${improved.length} file(s) have fewer errors than the baseline. Run \`yarn typecheck --update\` to lock that in.`);
}
